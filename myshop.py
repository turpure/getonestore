...
def getseller():
    ...
    cur=conn.cursor()
    query="select username from user_name"
    # query="select distinct sellername from usShoesSeller where location like '%China%' or location like '%Hong%'"
    cur.execute(query)
...
class Putin(object):
    ...
    def putin(self):
        ...
        qury='select itemid from shop_items where itemid=%s'
        sqlinsert='insert into shop_items values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        while True:
            try:
                l=savequeue.get()
                if l:
                    if not self.cur.execute(qury,(l['itemid'],)):
                        self.cur.execute(sqlinsert,(l['country'],l['currency'],l['hitcounter'],
                                                           ...
                                                           l['deltatitle'],l['deltasold'],l['deltahit'],l['deltaprice'],l['listingstatus']
                                                           ))
                        self.conn.commit()
